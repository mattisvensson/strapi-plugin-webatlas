...
export const RouteItem = forwardRef<HTMLDivElement, RouteItemProps>(
	(
		...
		const itemPath = useMemo(() => {
			...
			const parentPath = parentNavItem
				? `${parentNavItem.clientModifications?.path || parentNavItem.route.path}/`
				: ''
		...
		const removeNavItemModifications = (
			navItems: NestedNavItem[] | undefined,
		...
		useEffect(() => {
			...
			const shouldSyncPath =
				item.clientModifications?.path !== itemPath &&
		...
		return (
			<Box ref={wrapperRef} style={elStyle}>
				<Box
					background="neutral0"
				...
				>
					<Flex justifyContent="space-between" gap={4}>
						<Flex gap={4}>
							...
							<Box width="1px" height="32px" background="neutral150" />
							<RouteItemIcon type={item.route.type} />
							<Flex gap={2}>
								<Typography fontWeight="bold">
									{item.clientModifications?.title
										? item.clientModifications.title
										: item.route.title}
								</Typography>
								{itemPath && <Typography textColor="neutral400">/{itemPath}</Typography>}
							</Flex>
