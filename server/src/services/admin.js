...
export default ({ strapi }) => ({
	...
	async getRelatedRoute(documentId) {
		var _a
		try {
			return await ((_a = strapi.db) === null || _a === void 0
				? void 0
	...
	async getProhibitedRouteIds(documentId) {
		try {
			...
			const descendants = (route === null || route === void 0 ? void 0 : route.documentId)
				? await getRouteDescendants(route.documentId)
				: []
			const nonInternalRouteIds = await getNonInternalRouteIds()
			const prohibitedRouteIds = [...descendants, ...nonInternalRouteIds]
			;(route === null || route === void 0 ? void 0 : route.documentId) &&
				prohibitedRouteIds.push(route.documentId)
	...
	async getNavigation(documentId, variant) {
		try {
			...
			if (documentId) {
				...
				if (!navigation) throw new Error('Navigation not found')
				if (variant) {
					navigation = navigation.map((nav) => buildStructuredNavigation(nav, variant))
				}
			}
			return navigation
		} catch (e) {
	...
	async createNavigation(name, visible) {
		try {
