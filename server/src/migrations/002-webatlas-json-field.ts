...
export default {
  ...
  async up(strapi: Core.Strapi) {
    ...
    for (const contentType of enabledContentTypes) {
      const tableName: string = (strapi.db.metadata.get(contentType.uid) as any)?.tableName
      ...
      if (!hasAnyOldCol) {
        ...
        strapi.log.warn(`[webatlas] New "webatlas" column not found in ${tableName} — schema may not have synced yet. Skipping data migration, old columns will still be dropped.`);
      } else {
        ...
        strapi.log.info(`[webatlas] Migrated ${migratedCount} / ${rows.length} rows in ${tableName}`);
      }
